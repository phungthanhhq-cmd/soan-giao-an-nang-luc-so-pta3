...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({
  ...
  const handleTestKey = async () => {
    ...
    setTestResult(null);
    try {
      // Test directly with @google/genai SDK on client
      ...
      if (res.text) {
        setTestResult({
          success: true,
          message: "Tuyệt vời! API Key hoàn toàn hợp lệ và kết nối với Google AI thành công 100%.",
        });
      } else {
        setTestResult({ success: false, message: "Google AI phản hồi rỗng. Vui lòng thử lại." });
      }
    } catch (err: any) {
      console.error("Test API Key failed:", err);
      let errMsg = err?.message || String(err);
      if (errMsg.includes("403") || errMsg.includes("401") || errMsg.includes("API_KEY_INVALID")) {
        errMsg = "Mã API Key chưa đúng hoặc chưa được Google kích hoạt. Vui lòng kiểm tra lại.";
      } else if (errMsg.includes("429") || errMsg.includes("quota")) {
        errMsg = "API Key đã hết lượt gọi trong phút này (Quota Exceeded). Vui lòng đợi 1-2 phút.";
      }
      setTestResult({ success: false, message: `Lỗi kết nối: ${errMsg}` });
    } finally {
      setTestingKey(false);
    }
  };
          ...
          <div className="space-y-2">
            ...
            <div className="pt-1">
              <button
                ...
                disabled={testingKey || (!inputKey.trim() && !apiKey.trim())}
                className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl border border-slate-300 transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer active:scale-95"
              >
                {testingKey ? (
                  <>
                    <span className="animate-spin inline-block w-3.5 h-3.5 border-2 border-indigo-600 border-t-transparent rounded-full mr-1"></span>
                    <span>Đang kiểm tra kết nối với Google AI...</span>
                  </>
                ) : (
                  <>
                    <Sparkles size={14} className="text-indigo-600" />
                    <span>KIỂM TRA KẾT NỐI API KEY NGAY</span>
                  </>
                )}
              </button>
              ...
              {testResult && (
                <div
                  className={`mt-2 p-3 rounded-xl text-xs font-medium border animate-in fade-in ${
                    testResult.success
